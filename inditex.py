...
# Başlık ve tanımları kaldırdım, sadece kartlar olacak
st.markdown('<div class="card-container">', unsafe_allow_html=True)

# Kullanıcının tıkladığı kartı saklamak için session state
if "flipped_cards" not in st.session_state:
    st.session_state.flipped_cards = {}

# Kartların render edilmesi
for word, definition in sozluk.items():
    if word not in st.session_state.flipped_cards:
        st.session_state.flipped_cards[word] = False

    card_flipped = st.session_state.flipped_cards[word]
    button_id = f"flip_{word}"

    # Kartın önü ve arkası, tıklanınca değişim
    if st.button(f"{word}", key=button_id):
        st.session_state.flipped_cards[word] = not card_flipped

    # Kartın render edilmesi
    if st.session_state.flipped_cards[word]:
        st.markdown(f"""
...
# Kartlar konteynerini kapatalım
